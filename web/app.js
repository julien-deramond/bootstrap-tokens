...
/**
 * Open the theme as a running project in a browser sandbox.
 *
 * The export dialog ends where the interesting part starts: a `custom.scss` and four
 * numbered steps, and the steps are where people fall off. This skips them — one click and
 * the theme is a Vite dev server compiling Bootstrap's own source Sass, with a page you can
 * click around in and StackBlitz's *Download project* as the way out.
 *
 * The files are the ones `bstokens init --template vite` writes, from the same generator, so
 * the sandbox and the CLI cannot describe different projects.
 *
 * A hand-built form POST rather than `@stackblitz/sdk`: this repository has no runtime
 * dependencies and does not want one for twenty lines of `<input type="hidden">`. The form
 * is built, submitted and thrown away inside the click handler, which is also what makes the
 * privacy claim beside the button true — there is no request until someone asks for one.
 */
function openInStackblitz() {
  const name = activeTheme(state.store)?.name ?? 'Bootstrap theme'
  ...
  const files = projectFiles({
    ...
    name,
    version: state.meta.bootstrap,
    template: 'vite'
  })

  const form = document.createElement('form')
  form.method = 'POST'
  // Opens on the theme itself, which is the file someone will want to edit first.
  form.action = 'https://stackblitz.com/run?file=src%2Fscss%2Fstyles.scss'
  form.target = '_blank'
  form.rel = 'noopener'
  ...
  // `node` is the WebContainer template: it installs from package.json and runs the
  // `stackblitz.startCommand` the generated project declares.
  field('project[template]', 'node')
  field('project[title]', name)
  field(
    'project[description]',
    `A Bootstrap v6 theme, generated by bootstrap-tokens. Bootstrap ${state.meta.bootstrap}.`
  )
  for (const [path, contents] of Object.entries(files)) field(`project[files][${path}]`, contents)

  document.body.append(form)
  form.submit()
  form.remove()

  const button = $('#stackblitz')
  button.textContent = 'Opening StackBlitz…'
  setTimeout(() => (button.textContent = 'Open in StackBlitz'), 2500)
}
...
function wire() {
  ...
  $('#stackblitz').addEventListener('click', openInStackblitz)
